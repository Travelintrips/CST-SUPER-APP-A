...
// ─────────────────────────────────────────────
productTemplatesRouter.get("/", async (_req: Request, res: Response) => {
  try {
    const rows = await db
      .select()
...
productTemplatesRouter.get("/:key", async (req: Request, res: Response) => {
  try {
    ...
    let rows;
    if (!isNaN(byId)) {
      rows = await db.select().from(productTemplatesTable).where(eq(productTemplatesTable.id, byId));
    } else {
      rows = await db.select().from(productTemplatesTable).where(eq(productTemplatesTable.categoryKey, key));
    }
    if (!rows.length) {
      // fallback to general
      const gen = await db.select().from(productTemplatesTable).where(eq(productTemplatesTable.categoryKey, "general"));
      if (!gen.length) return res.status(404).json({ message: "Template tidak ditemukan" });
      return res.json(gen[0]);
    }
    res.json(rows[0]);
  } catch (err) {
    res.status(500).json({ message: String(err) });
