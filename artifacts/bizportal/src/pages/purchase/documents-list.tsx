...
export default function PurchaseDocumentsListPage({ kind }: Props) {
  const { data: docs } = useListPurchaseDocuments({ kind });
  ...
  return (
    <AppShell>
      <div className="flex flex-col gap-6">
        ...
        <Card>
          <CardHeader><CardTitle>Daftar {title}</CardTitle></CardHeader>
          <CardContent>
            <Table>
              <TableHeader>
                <TableRow>
              ...
              <TableBody>
                ...
                {(!docs || docs.length === 0) && (
                  <TableRow>
                    <TableCell colSpan={isRfq ? 5 : 7} className="text-center text-muted-foreground py-8">
                      Belum ada dokumen.
                    </TableCell>
                  </TableRow>
                )}
