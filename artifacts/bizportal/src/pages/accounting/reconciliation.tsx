                    ...
                    <TableBody>
                        ...
                        return (
                          <TableRow key={line.id} className={isRec ? "bg-emerald-50/60" : ""}>
                            ...
                            <TableCell className="text-right font-mono text-xs">
                              {line.debit > 0 && <span className="text-blue-700 font-semibold">{idr(line.debit)}</span>}
                            </TableCell>
                            <TableCell className="text-right font-mono text-xs">
                              {line.credit > 0 && <span className="text-emerald-700 font-semibold">{idr(line.credit)}</span>}
                            </TableCell>
                            <TableCell>
                              {isRec ? (
                      ...
                      {reconciledRows.length > 0 && (
                        <TableRow className="bg-emerald-50/40">
                          ...
                          <TableCell className="text-right font-mono text-xs text-emerald-700">{idr(reconciledDebit)}</TableCell>
                          <TableCell className="text-right font-mono text-xs text-emerald-700">{idr(reconciledCredit)}</TableCell>
                          <TableCell></TableCell>
                        </TableRow>
                      )}
...
function GSheetTab() {
  ...
  function handleExport() {
    ...
    exportXlsx("Rekonsiliasi_GSheet",
      ...
      result.results.map((r) => [
        r.entryNumber,
        new Date(r.entryDate).toLocaleDateString("id-ID"),
        r.debit || "",
        r.credit || "",
        r.description ?? "",
        r.key,
  ...
  const displayedRows = useMemo(() => {
    if (!result) return [];
    if (filterStatus === "all") return result.results;
        ...
        <CardContent className="p-4">
          <div className="flex gap-2 text-blue-800">
            ...
            <div className="text-sm space-y-1">
              ...
              <p>Kolom A=Tanggal, B=Debit, C=Kredit, D=Keterangan, <strong>E=Unique Key</strong> (hasil formula), <strong>F=Status</strong> (akan diisi otomatis)</p>
              <p>Unique Key menggunakan rumus: <code className="bg-blue-100 px-1 rounded text-xs">yyyymmdd_jumlah_IN/OUT</code></p>
              <p>Pastikan Service Account sudah diberi akses Editor ke spreadsheet.</p>
            </div>
          </div>
        </CardContent>
      </Card>
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                Dari URL: docs.google.com/spreadsheets/d/<strong>ID_INI</strong>/edit
              </p>
            </div>
            <div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1 flex items-end justify-end gap-2">
              ...
              <Button onClick={handleRun} disabled={loading} className="min-w-40">
                {loading ? (
      ...
      {result && (
        <>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-xs text-muted-foreground font-medium uppercase tracking-wide">Total Entry</CardTitle>
              </CardHeader>
              <CardContent className="pt-0">
            ...
            <Card className="border-emerald-300">
              <CardHeader className="pb-2">
                <CardTitle className="text-xs text-emerald-700 font-medium uppercase tracking-wide flex items-center gap-1">
                  <CheckCircle2 className="h-3.5 w-3.5" />Cocok
                </CardTitle>
              </CardHeader>
              <CardContent className="pt-0">
                <p className="text-2xl font-bold text-emerald-700">{result.summary.matched}</p>
              </CardContent>
            </Card>
            <Card className="border-amber-300">
              <CardHeader className="pb-2">
                <CardTitle className="text-xs text-amber-700 font-medium uppercase tracking-wide flex items-center gap-1">
                  <AlertTriangle className="h-3.5 w-3.5" />Duplikat
                </CardTitle>
              </CardHeader>
              <CardContent className="pt-0">
                <p className="text-2xl font-bold text-amber-700">{result.summary.duplicate}</p>
              </CardContent>
            </Card>
            <Card className="border-red-300">
              <CardHeader className="pb-2">
                <CardTitle className="text-xs text-red-700 font-medium uppercase tracking-wide flex items-center gap-1">
                  <XCircle className="h-3.5 w-3.5" />Tidak Ditemukan
                </CardTitle>
              </CardHeader>
              <CardContent className="pt-0">
                <p className="text-2xl font-bold text-red-700">{result.summary.notFound}</p>
              </CardContent>
            </Card>
          </div>

          <div className="text-xs text-muted-foreground text-right">
            {result.summary.updated} baris diperbarui di Google Sheet
          </div>
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead>Status</TableHead>
                      <TableHead className="text-right">Baris GS</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {displayedRows.length === 0 ? (
                      ...
                      return (
                        <TableRow
                          key={r.id}
                          className={
                            isCocok ? "bg-emerald-50/40" :
                            isDuplikat ? "bg-amber-50/40" :
                            "bg-red-50/30"
                          }
                        >
                          ...
                          <TableCell className="text-right font-mono text-xs">
                            {r.debit > 0 && <span className="text-blue-700 font-semibold">{idr(r.debit)}</span>}
                          </TableCell>
                          <TableCell className="text-right font-mono text-xs">
                            {r.credit > 0 && <span className="text-emerald-700 font-semibold">{idr(r.credit)}</span>}
                          </TableCell>
    ...
    <AppShell>
      <div className="space-y-6 p-6">
        ...
        <div className="flex gap-1 border-b">
          ...
          >
            <GitMerge className="h-4 w-4 inline mr-1.5 -mt-0.5" />
            Manual
          </button>
          <button
